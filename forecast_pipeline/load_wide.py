...
def load_feature_map() -> dict:
    """feature_cols.json: ['BGT-N1_黑球北1', ...] -> {pointId: 特征名}"""
    with open(BASE_DIR / "feature_cols.json", encoding="utf-8") as f:
        feats = json.load(f)
    mapping = {}
    for feat in feats:
...
def load_wide_history(engine, gateway_mac: str = GATEWAY_MAC, device_id: str = DEVICE_ID,
    ...
    """从 device_data_save 长表读取并转为 288行×27特征 宽表 (含 '时间' 列)

    步骤:
      1. 查询最近 history_hours 小时内, 模型所需 27 个 pointId 的记录
      2. value(varchar) 转数值
    ...
    返回: DataFrame[时间, 27特征], 时间严格连续5min
    ...
    long_gap = []
    for _, sub in miss.groupby(groups):
        if bool(sub.iloc[0]) and len(sub) > 6:
