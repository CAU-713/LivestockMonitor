...
输入：已经按 5 分钟聚合的宽表 DataFrame，至少包含最近连续 288 行。
输出：未来 72 个 5 分钟点 × 27 个变量的长表 DataFrame（共 1944 行）。
...
class XLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        centered = (x - seq_mean).permute(0, 2, 1)
        prediction = self.Linear(centered).permute(0, 2, 1)
        return prediction + seq_mean
...
class EnvironmentPredictor:
    ...
    def __init__(
        ...
        self.scaler_path = Path(scaler_path) if scaler_path else base / "scaler.pkl"
        self.feature_path = Path(feature_path) if feature_path else base / "feature_cols.json"
        self.device = torch.device(device)

        self._check_asset_files()

        with self.feature_path.open("r", encoding="utf-8") as f:
            self.features = json.load(f)

        if len(self.features) != 27:
            raise ValueError(f"feature_cols.json 应包含27个特征，实际为{len(self.features)}个")
        if len(set(self.features)) != len(self.features):
            raise ValueError("feature_cols.json 中存在重复特征")

        # pickle 只能加载可信文件；这里应使用训练阶段生成的 scaler.pkl。
        with self.scaler_path.open("rb") as f:
            self.scaler = pickle.load(f)

        scaler_features = getattr(self.scaler, "n_features_in_", None)
        if scaler_features is not None and int(scaler_features) != len(self.features):
            raise ValueError(
                f"scaler特征数为{scaler_features}，feature_cols.json为{len(self.features)}，二者不一致"
            )

        self.model = XLinear(SEQ_LEN, PRED_LEN).to(self.device)
        checkpoint = self._load_checkpoint()
        state_dict = checkpoint.get("state_dict", checkpoint)
        self.model.load_state_dict(state_dict, strict=True)
        self.model.eval()

        ckpt_features = checkpoint.get("feat_names") if isinstance(checkpoint, dict) else None
        if ckpt_features is not None and list(ckpt_features) != self.features:
            raise ValueError("模型检查点中的特征顺序与 feature_cols.json 不一致")

    def _check_asset_files(self) -> None:
        for path in (self.model_path, self.scaler_path, self.feature_path):
            if not path.is_file():
    ...
    def _prepare_input(
        ...
        missing = [f for f in self.features if f not in history_df.columns]
        if missing:
            raise ValueError(f"缺少模型特征列：{missing}")

        data = history_df[[time_col] + self.features].copy()
        ...
        if data[self.features].isna().any().any():
            counts = data[self.features].isna().sum()
            bad_cols = counts[counts > 0].to_dict()
            raise ValueError(f"输入含缺失或非数值数据：{bad_cols}")

        values = data[self.features].to_numpy(dtype=np.float64)
    ...
    def predict(
        ...
        ----------
        history_df:
            已经按5分钟聚合的宽表。至少288行，必须包含时间列和27个特征列。
        ...
        raw = data[self.features].to_numpy(dtype=np.float64)
        ...
        if pred_scaled.shape != (PRED_LEN, len(self.features)):
            raise RuntimeError(
                f"模型输出形状错误：{pred_scaled.shape}，期望({PRED_LEN}, {len(self.features)})"
            )

        pred_raw = self.scaler.inverse_transform(pred_scaled)
        pred_raw = self._clip_physical_ranges(pred_raw, self.features)
        ...
        expected_rows = PRED_LEN * len(self.features)
...
if __name__ == "__main__":
    # 本地演示：deploy目录中准备 demo_input.csv 后执行 python predict.py
