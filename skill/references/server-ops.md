# LivestockMonitor 服务器运维速查手册（完整版）

> 本文档是 skill `livestock-deploy` 的详细参考文件。所有命令均带超时保护。
...
| Docker Compose 前端内存限制 | `800m`（`mem_limit`），swap `1200m` |
...
# WebSocket 端点验证（curl 返回 426 Upgrade Required 是正常的）
ssh -i ~/.ssh/id_ed25519 -o ConnectTimeout=10 ubuntu@120.53.24.48 \
...
```bash
# 推荐：用 rsync 上传（排除 node_modules/.next）
...
> **重要**：后端镜像包含 torch + CUDA 依赖，约 8.65GB。首次构建（无缓存）需 5-8 分钟，
> `timeout` 必须设 **600 秒以上**。如果磁盘空间不足（<10GB可用），构建会因 pip 安装失败。

```bash
# 用 rsync 上传（排除 __pycache__/.venv）
...
> **注意**：服务器上 `/root/LivestockMonitor` **没有 git 仓库**，代码通过 `scp/rsync` 直接上传文件同步。
> 本地仍用 git 管理（推送到 GitHub），但服务器不 pull，而是直接覆盖文件。
...
# 2. 用 rsync 上传修改的文件到服务器（推荐）
rsync -avz --delete --exclude='node_modules' --exclude='.next' \
  -e "ssh -i ~/.ssh/id_ed25519 -o ConnectTimeout=10" \
  ./frontend/ ubuntu@120.53.24.48:/root/LivestockMonitor/frontend/

# 也可以用 scp（慢，会包含 node_modules）
...
### rsync 单文件上传（仅改了一个文件时）

```bash
# 例：只改了告警页面
rsync -avz -e "ssh -i ~/.ssh/id_ed25519 -o ConnectTimeout=10" \
  ./frontend/app/(main)/alerts/page.tsx \
  ubuntu@120.53.24.48:/root/LivestockMonitor/frontend/app/(main)/alerts/page.tsx
```

### scp 权限问题

> 服务器 `/root/` 目录默认属 `root:root`，`ubuntu` 用户无写权限。
> 如果 scp/rsync 报 `Permission denied`，需先在服务器上修改目录权限。
> **建议**：每次新会话部署前，先执行一次 `chown` 命令确保权限正常，避免上传失败浪费时间。
...
# 再重建（timeout 600）
ssh -i ~/.ssh/id_ed25519 -o ConnectTimeout=10 ubuntu@120.53.24.48 \
  "timeout 600 bash -c 'cd /root/LivestockMonitor && sudo docker compose build backend 2>&1'"
```
